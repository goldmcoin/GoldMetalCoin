...
#define URI_SCHEME "goldmetalcoin"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a GMC address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no goldmetalcoin: URI
...
{
    // Convert goldmetalcoin:// to goldmetalcoin:
    //
    //    Cannot handle this later, because goldmetalcoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open goldmetalcoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "GoldMetalCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for GoldMetalCoin.lnk
...
{
    return GetAutostartDir() / "goldmetalcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a goldmetalcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=GoldMetalCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the goldmetalcoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add goldmetalcoin app to startup item list
